	...
	protected Cell direction()
    {
    	...
    	while(foodFound!=true && fullMake!=true)
    	{
	    	...
	    	for(int i= cmptXmin ; i <= cmptXmax ; i++ )
	    	{
	    		...
	    		//	*X*
	    		//	***
		    	if(CellLife.theWorld.getCell(this.position.x+i, this.position.y-cmptYmin).getVegetables() !=0)
		    	{
		    		foodFound=true;	
	    		...
	    		//	*X*
	    		//	*** <== check this raw
		    	if(CellLife.theWorld.getCell(this.position.x+i, this.position.y+cmptYmax).getVegetables()!=0)
		    	{
		    		foodFound=true;	
		    		positionX=this.position.x+i;
	    	...
	    	for(int i = cmptYmin ; i <= cmptYmax ; i++ )
	    	{
	    		...
	    		//	IX*
	    		//	I**
	    		if(CellLife.theWorld.getCell(this.position.x-cmptXmin, this.position.y+i).getVegetables() !=0)
	    		{
		    		foodFound=true;	
	    		...
	    		//	*XI
	    		//	**I
	    		if(CellLife.theWorld.getCell(this.position.x+cmptXmax, this.position.y+i).getVegetables() !=0 )
	    		{
		    		foodFound=true;	
